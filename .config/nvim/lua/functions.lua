...
M.scratch_to_quickfix = function(close_qf) local items, bufnr = {}, vim.api.nvim_get_current_buf()
...
M.scratch=function() vim.bo.buftype = "nofile" vim.bo.bufhidden = "wipe" vim.bo.swapfile = false end
M.pre_search=function() if vim.bo.filetype == "netrw" then return vim.b.netrw_curdir, _G.basic_excludes, {} else return vim.fn.getcwd(), _G.ext_excludes, {} end end
M.extcmd=function(cmd, qf, close_qf, novsplit) local out = vim.fn.systemlist(cmd) if not out or #out == 0 then return end
  vim.cmd(novsplit and "enew" or "vnew") vim.api.nvim_buf_set_lines( 0, 0, -1, false, out) M.scratch() if qf then M.scratch_to_quickfix(close_qf) end end
